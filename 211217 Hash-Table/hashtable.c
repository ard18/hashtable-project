...
static int hash_table_index(hash_table *ht, char *key){		// compute hashtable index based on hash value
															// this is a static function, i.e., it can be called only within the file
															// hashtable.c
															// This is done because this function is meant to be used only by
															// the other functions in this file, not by those in the hashmain file
...
char *linearSearch(hash_table *ht, char *key) {				// look for a word in the hashtable using linear search
    ...
    for (int i = 0; i < ht->size; i++) {
    	tmp = ht->elements[i];
    	while(tmp!=NULL && strcmp(tmp->key, key)!=0){		// traverse the linked list in case of collision
    		tmp=tmp->next;
    	}
    	if(tmp!=NULL){
    		return tmp->key;
